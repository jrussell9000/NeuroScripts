...
##$my_script/my_modelfit -inputfile ${camino_dwi}.Bfloat -bgmask ${strip_prefix}_bin.nii.gz -schemefile ${PROCESS}/SCHEME/${eddy}*.scheme -inputdatatype float -outputdatatype float -model restore -sigma ${sigma} -outputfile ${camino_dti}.Bfloat 2> ${prefix}_tmp.txt
