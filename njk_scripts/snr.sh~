...
my_script=/Volumes/Vol6/YouthPTSD/scripts/njk_scripts
sh ${my_script}/my_image2voxel -4dimage ${strip_prefix}.nii.gz > ${camino_dwi}.Bfloat
...
fslmaths ${strip_prefix}_DTK_fa.nii -bin -dilM -dilM -dilM -dilM -dilM -add 1 -uthr 1.5 ${strip_prefix}_DTK_background;

echo ~~~Calculate SNR with 1 b0 image~~~
estimatesnr -inputfile ${camino_dwi}.Bfloat -schemefile ${scheme_txt} -bgmask ${strip_prefix}_DTK_fa_wm.nii.gz -noiseroi ${strip_prefix}_DTK_background.nii.gz > ${strip_prefix}_snr.txt
