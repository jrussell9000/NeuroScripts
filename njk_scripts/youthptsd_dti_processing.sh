...
##echo ~~~Make FMAP~~~;
#sh $my_script/create_fmap.sh $raw $PROCESS $manual_mask $mask_prefix $fmap_prefix $twodfast_raw $i;
#echo

echo ~~~Eddy Correction~~~;
my_script=/Volumes/Vol6/YouthPTSD/scripts/njk_scripts
echo sh $my_script/eddy_script.sh $PROCESS $dti_prefix $eddy_prefix $subj;
sh $my_script/eddy_script.sh $PROCESS $dti_prefix $eddy_prefix $subj;
echo

echo ~~~Fieldmap Correction~~~;
my_script=/Volumes/Vol6/YouthPTSD/scripts/njk_scripts
sh $my_script/fmap_script.sh $raw $PROCESS $pepolar $eddy_prefix $fmap_prefix $subj;
echo

#echo ~~~Remove Rim~~~;

echo sh $my_script/remove_rim.sh ${PROCESS} ${num} ${b0} ${bvalue} ${grad_dir_txt} ${corrected_prefix} ${strip_prefix} $subj;
sh $my_script/remove_rim.sh ${PROCESS} ${num} ${b0} ${bvalue} ${grad_dir_txt} ${corrected_prefix} ${strip_prefix} $subj;
echo

#echo ~~~Calculate SNR~~~;
my_script=/Volumes/Vol6/YouthPTSD/scripts/njk_scripts
echo sh $my_script/snr.sh $PROCESS ${num} ${b0} ${bvalue} ${grad_dir_txt} ${snr_txt} ${bvec_txt} ${bval_txt} ${scheme_txt} ${eddy_prefix} ${strip_prefix} ${camino_dwi} $subj;
sh $my_script/snr.sh $PROCESS ${num} ${b0} ${bvalue} ${grad_dir_txt} ${snr_txt} ${bvec_txt} ${bval_txt} ${scheme_txt} ${eddy_prefix} ${strip_prefix} ${camino_dwi} $subj;

sigma=`cat ${snr_txt} | grep ^${strip_prefix} | awk 'BEGIN{FS=","}{print $3}'`;
sigma=`echo $sigma | awk 'BEGIN{FS=" "}{print $1}'`;
echo Sigma is: ${sigma};
echo

#echo ~~~Tensor calculation using RESTORE~~~;
my_script=/Volumes/Vol6/YouthPTSD/scripts/njk_scripts
echo sh $my_script/tensor_rest.sh ${PROCESS} ${sigma} ${camino_dwi} ${strip_prefix} ${scheme_txt} ${camino_dti} ${dt_prefix} ${subj};
sh $my_script/tensor_rest.sh ${PROCESS} ${sigma} ${camino_dwi} ${strip_prefix} ${scheme_txt} ${camino_dti} ${dt_prefix} ${subj};
echo
