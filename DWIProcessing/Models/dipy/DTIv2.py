...
def fit_dti_dipy(subjroot, input_dwi, input_bval, input_bvec, output_dir, fit_type='', mask='',
                 bmax='', mask_tensor='T'):
    ...
    # Define output imgs
    output_evecs = output_dir / (subjroot + '_eigenvectors.nii.gz')
    output_tensor = output_dir / (subjroot + '_tensor.nii.gz')
    dti_tensor_spd = output_dir / (subjroot + '_tensor_spd.nii.gz')
    output_tensor_norm = output_dir / (subjroot + '_tensor_norm.nii.gz')
    dti_tensor_spd_masked = output_dir / (subjroot + '_tensor_spd_masked.nii.gz')
    norm_mask = output_dir / (subjroot + '_norm_mask.nii.gz')
    output_V1 = output_dir / (subjroot + '_V1.nii.gz')
    output_V2 = output_dir / (subjroot + '_V2.nii.gz')
    output_V3 = output_dir / (subjroot + '_V3.nii.gz')
    output_L1 = output_dir / (subjroot + '_L1.nii.gz')
    output_L2 = output_dir / (subjroot + '_L2.nii.gz')
    output_L3 = output_dir / (subjroot + '_L3.nii.gz')

    output_fa = output_dir / (subjroot + '_FA.nii.gz')
    output_md = output_dir / (subjroot + '_MD.nii.gz')
    output_rd = output_dir / (subjroot + '_RD.nii.gz')
    output_ad = output_dir / (subjroot + '_AD.nii.gz')

    output_res = output_dir / (subjroot + '_residuals.nii.gz')
    ...
    os.chdir(output_dir)
    subprocess.run(['TVFromEigenSystem', '-basename', subjroot, '-type', 'FSL', '-out', output_tensor])
    subprocess.run(['TVtool', '-in', output_tensor, '-scale', '1000.00', '-out', output_tensor])
    os.system('rm -rf *_V* *_L*')

    # Make the tensor file symmetric, positive definite:
    # http://dti-tk.sourceforge.net/pmwiki/pmwiki.php?n=Documentation.BeforeReg
    subprocess.run(['TVtool', '-in', output_tensor, '-spd', '-out', dti_tensor_spd])

    # If we're brain masking the tensor file...
    if mask_tensor == 'T':
        # Compute the 'size' of each tensor as it's 'norm'
        subprocess.run(['TVtool', '-in', dti_tensor_spd, '-norm', '-out', output_tensor_norm])
        # Create brain mask
        subprocess.run(['BinaryThresholdImageFilter', output_tensor_norm, norm_mask, '0.01', '3.0', '1', '0'])
        # Mask the tensor image
        subprocess.run(['TVtool', '-in', dti_tensor_spd, '-mask', norm_mask, '-out', dti_tensor_spd_masked])
        # Convert the tensor to FSl format (?)
        subprocess.run(['TVEigenSystem', '-in', dti_tensor_spd_masked, '-type', 'FSL'])

        # Calculate Eigenvectors and Eigenvalues, FA, MD, RD, AD
        subprocess.run(['TVtool', '-in', dti_tensor_spd_masked, '-fa', '-out', output_fa])
        subprocess.run(['TVtool', '-in', dti_tensor_spd_masked, '-rd', '-out', output_rd])
        subprocess.run(['TVtool', '-in', dti_tensor_spd_masked, '-ad', '-out', output_ad])
        subprocess.run(['TVtool', '-in', dti_tensor_spd_masked, '-tr', '-out', output_md])
        subprocess.run(['fslmaths', output_md, '-div', '3.0', output_md])
...
def main(ses_dir):
    ...
    fit_dti_dipy(subjroot, input_dwi, str(input_bval), str(input_bvec), output_dir, fit_type='RESTORE',
