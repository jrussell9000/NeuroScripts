...
wmfod_templ = Path('/fast_scratch/jdr/mrtrix/cross_sec/wmfod_template.mif')
...
def createFODlinks():
    ...
    for mask in bidsproc_dir.glob('**/preprocessed/sub-*_ses-01_mask_ppd.mif'):
...
def regfod2Popltempl(ses_dir):
    ...
    subprocess.run(['mrregister', '-force', wmfod_norm, '-mask1', dwi_mask_ppd, wmfod_templ, '-nl_warp',
...
def transformMask(ses_dir):
    ...
    subprocess.run(['mrtransform', '-force', dwiMaskUps, '-warp', subj2templwarp_mif, '-interp', 'nearest',
...
def computeTemplMask():
    mifstrs = []
    for mif in bidsproc_dir.glob('**/ses-01/dwi/processed/mrtrix/ss3t/sub-*_ses-01_mask_ppd_reg2CrossSectempl.mif'):
        print(mif.name)
    ...
    os.system('mrmath -force ' + common_mask_paths + ' min ' + str(mask_template) + ' -datatype' + ' bit')
    return mask_template


# Step 12. Compute a white matter template analysis fixel mask
def segmentFODTempl(mask_template):
    fixel_mask = template_dir / 'fixel_mask'
    if fixel_mask.exists():
        shutil.rmtree(fixel_mask)
    subprocess.run(['fod2fixel', '-force', '-mask', mask_template, '-fmls_peak_value', '0.06',
                    wmfod_templ, fixel_mask])
    return fixel_mask
...
def reorientFixels(ses_dir):
    ...
    subj2templwarp = ss3t_dir / (subjroot + '_subj2templwarp.mif')
    fixel_reg2templ_notreoriented = ss3t_dir / 'fixel_reg2templ_notreoriented'
    fixel_reg2templ_dir = ss3t_dir / 'fixel_reg2templ'
    if fixel_reg2templ_dir.exists():
        shutil.rmtree(fixel_reg2templ_dir)
    subprocess.run(['fixelreorient', '-force', fixel_reg2templ_notreoriented, subj2templwarp,
                    fixel_reg2templ_dir])

createFODlinks()
createPoplTempl(template_dir, fod_input_dir, mask_input_dir)
...
with parallel_backend("loky", inner_max_num_threads=8):
    results = Parallel(n_jobs=8, verbose=1)(
        delayed(regfod2Popltempl)(ses_dir) for ses_dir in sorted(ses_dirs))

with parallel_backend("loky", inner_max_num_threads=8):
    results = Parallel(n_jobs=8, verbose=1)(
        delayed(transformMask)(ses_dir) for ses_dir in sorted(ses_dirs))

computeTemplMask()
segmentFODTempl(mask_template)

with parallel_backend("loky", inner_max_num_threads=8):
    results = Parallel(n_jobs=8, verbose=1)(
        delayed(warpFODs2Templ)(ses_dir) for ses_dir in sorted(ses_dirs))

with parallel_backend("loky", inner_max_num_threads=8):
    results = Parallel(n_jobs=8, verbose=1)(
        delayed(segmentSubjFODs)(ses_dir) for ses_dir in sorted(ses_dirs))

with parallel_backend("loky", inner_max_num_threads=8):
    results = Parallel(n_jobs=8, verbose=1)(
        delayed(reorientFixels)(ses_dir) for ses_dir in sorted(ses_dirs))
