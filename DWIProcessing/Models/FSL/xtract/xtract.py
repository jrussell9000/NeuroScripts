...
xtract_dir = Path('/scratch/jdrussell3/fsl/xtract/')
xtractlogs_dir = Path('/scratch/jdrussell3/fsl/xtract/logs')
if xtractlogs_dir.exists():
    shutil.rmtree(xtractlogs_dir)
xtractlogs_dir.mkdir()

bedpostx_dirs_str = (str(bedpostx_dir) for bedpostx_dir in sorted(Path(xtract_dir).glob('sub-*')))
...
def run_xtract(bedpostxsubj_dir):
    ...
    with open(logfile, 'a') as log:
        ...
        subprocess.run(['xtract', '-bpx', str(bedpostxsubj_dir), '-out', str(xtractout_dir),
                        '-species', 'HUMAN', '-stdwarp', str(std2diff), str(diff2std), '-gpu'],
                       env=newenv, stdout=log, stderr=subprocess.STDOUT)


bedpostxsubj_dirs = (bedpostxsubj_dir for bedpostxsubj_dir in sorted(Path(xtract_dir).glob('sub-*')))
