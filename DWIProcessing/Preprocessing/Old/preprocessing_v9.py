...
ses_dirs_str = (str(ses_dir) for ses_dir in sorted(bidsmaster_dir.glob('*/ses-*')) if Path(ses_dir / 'dwi').exists())
...
def dwi_corr(ses_dir):
    ...
    with open(logfile, 'a') as log:
        ...
        # Define paths to eddy corrected volumes and bvecs
...
subjs = ['sub-106', 'sub-106', 'sub-107', 'sub-108', 'sub-108', 'sub-111', 'sub-112', 
         'sub-112', 'sub-114', 'sub-117', 'sub-117', 'sub-118', 'sub-118', 'sub-122', 
         'sub-122', 'sub-124', 'sub-124', 'sub-125', 'sub-125', 'sub-127', 'sub-128', 
         'sub-129', 'sub-129', 'sub-131', 'sub-131', 'sub-132', 'sub-133', 'sub-134', 
         'sub-134', 'sub-135', 'sub-135', 'sub-138', 'sub-139', 'sub-139', 'sub-140', 
         'sub-140', 'sub-141', 'sub-142', 'sub-145', 'sub-146', 'sub-147', 'sub-148', 
         'sub-149', 'sub-151', 'sub-153', 'sub-154', 'sub-155', 'sub-156', 'sub-157']

subj_dirs = (subj_dir for subj_dir in bidsmaster_dir.iterdir()
             if subj_dir.is_dir() and subj_dir.name in subjlist1of3)

# ses_dirs = (ses_dir for ses_dir in bidsmaster_dir.glob('*/ses-*') if Path(ses_dir / 'dwi').exists() and any(x in
#             ses_dir.parents[0].name) for x in subjs))
