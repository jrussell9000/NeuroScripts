...
# !!!!!!Problem: the gpu switch is reset to zero with each new subj_dir
# Possible fix: iterating over nested directories: https://stackoverflow.com/questions/19587118/iterating-through-directories-with-python
gpu_switch = 0
...
def dwi_corr(subj_dir):
    ...
    for ses_dir in sorted(ses_dirs):
        ...
        with open(logfile, 'a') as log:
            ...
            log.write("#----Now Starting Eddy Current Correction on GPU " +
                      str(gpu_switch) + "----#\n")
            log.flush()
            try:
