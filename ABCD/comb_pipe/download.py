...
def generate_parser():
    ...
    parser.add_argument(
        "-x", "--single-subject", dest="single_subject_id", type=str, required=False,
        help=("A single subject ID (starting with 'sub-') for whom inputs will be "
              "downloaded. Cannot be used with --subject-list.")
    )
    parser.add_argument(
...
def _cli():
    ...
    subject_list = get_subject_list(manifest_df, args.subject_list_file, args.single_subject_id)
...
def get_subject_list(manifest_df, subject_list_file, single_subject_id):
    """
    ...
    elif single_subject_id:
        print ('\tSubject:\t%s' % single_subject_id)
        subject_list.add(single_subject_id)
