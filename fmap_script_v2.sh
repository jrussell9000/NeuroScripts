  ...
  if [ $pepolar == 0 ]
    then
      ...
      for i in ${subject}; do
        ...
        echo Written:
        ls -ltrh ${PROCESS}/CORRECTED/${subject}*
      done

    else
      echo ~~~FMAP Correct for 1 pepolar~~~
      for i in ${subject}; do
        echo fieldmap_correction --beautify ${fmap_prefix}.nii 0.568 ${PROCESS}/CORRECTED/ ${eddy_prefix}.nii;
        fieldmap_correction --beautify ${fmap_prefix}.nii 0.568 ${PROCESS}/CORRECTED/ ${eddy_prefix}.nii;
        echo Written:
        ls -ltrh ${PROCESS}/CORRECTED/${subject}*
      done
  fi
