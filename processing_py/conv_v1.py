...
class BidsConv():
    ...
    def organize_dcms(self):
        ...
        rawscan_type = rawscan_dirname.split('.')[1]
        ...
        bidsscan_mode = self.scan2bidsmode(rawscan_type)
        ...
        bidsscan_echo = '_echo%e' if rawscan_type.__contains__(
            'DUAL_ECHO') else ''
        ...
        bidsscan_run_no = "_run-" + \
            str(rawscan_seqno) if rawscan_type.__contains__('EPI') else ''
        # ----bidsscan_tasklabel: if a functional (EPI) scan, the BIDS formatted name of the task
        bidsscan_tasklabel = ""
        if rawscan_type.__contains__('Perspective'):
            bidsscan_tasklabel = '_task-Perspective'
        elif rawscan_type.__contains__('n-Back'):
            bidsscan_tasklabel = "_task-n-Back"
        elif rawscan_type.__contains__('Resting'):
            bidsscan_tasklabel = '_task-Resting_state'
        # ---Diffusion-weighted scans
        # ----bidsscan.dwi.pedir: if a diffusion-weighted scan, the (semi-)BIDS formattedphase encoding direction
        bidsscan_dwi_pedir = ""
        if rawscan_type.__contains__('pepolar0'):
            bidsscan_dwi_pedir = "_dir-PA"
        elif rawscan_type.__contains__('pepolar1'):
        ...
        if any(x in rawscan_type for x in self.anatomicalscans):
            bidsscan_acqlabel = "_acq-" + rawscan_type.replace("_", "-")
        # ---Functional: no acquisition label
        elif any (x in rawscan_type for x in self.functionalscans):
            bidsscan_acqlabel = ""
        # ---Diffusion Weighted: the acquisition type
        elif any (x in rawscan_type for x in self.dwiscans):
            if rawscan_type.__contains__('HB'):
                bidsscan_acqlabel = "_acq-NODDI-multiband"
            else:
                bidsscan_acqlabel = "_acq-NODDI-singleband"
        # ---Fieldmaps: just replace the underscores
        elif any (x in rawscan_type for x in self.fieldmapscans):
            bidsscan_acqlabel = "_acq-" + rawscan_type.replace("_", "-")
