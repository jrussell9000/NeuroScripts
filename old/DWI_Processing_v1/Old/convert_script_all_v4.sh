...
cp_info() {
  cp "${RAW_INPUT_DIR}"/*"${SUBJ}"/dicoms/info.txt "${SUBJ_PATH}"/INFO/info.txt;
}
...
convert_t1() {
  ...
  for SCAN in "${RAW_INPUT_DIR}"/*"${SUBJ}"/dicoms/*bravo; do
    ...
    if [ -d "${SCAN}" ]; then
      ...
      decompress T1
      dcm2niix -b y -f T1 -o "${SUBJ_PATH}"/ANAT/ "${TMP_PATH}"/T1
    fi
  done
}
...
convert_t2() {
  ...
  for SCAN in "${RAW_INPUT_DIR}"/*"${SUBJ}"/dicoms/*3dir; do
    if [ -d "${SCAN}" ]; then
      ...
      decompress T2
      dcm2niix -b y -f T2 -o "${SUBJ_PATH}"/ANAT/ "${TMP_PATH}"/T2/
    fi
  done
}
...
convert_fmap() {
  ...
  for SCAN in "${RAW_INPUT_DIR}"/*"${SUBJ}"/dicoms/*fmap; do
    if [ -d "${SCAN}" ]; then
      ...
      "${FMRITOOLS_PATH}"/make_fmap.py "${TMP_PATH}"/FMAP/"$(basename "$SCAN")" "${SUBJ_PATH}"/FMAP/"${MAPTYPE}"_FMAP.nii 
...
convert_dti() {
  ...
  for SCAN in "${RAW_INPUT_DIR}"/*"${SUBJ}"/dicoms/*dti; do
    if [ -d "${SCAN}" ]; then
      ...
      decompress DTI
      dcm2niix -b y -f DTI -o "${SUBJ_PATH}"/DTI/ "${TMP_PATH}"/DTI/
    fi
  done
}
