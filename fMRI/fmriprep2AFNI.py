...
class fmriprep2AFNI():
    ...
    def loadconfounds(self):
        ...
        confounds_tsv = self.func_dir / str(self.subjroot +
                                            '_task-EPIDynamicFaces_desc-confounds_timeseries.tsv')

        if not confounds_tsv.exists():
            confounds_tsv = self.func_dir / str(self.subjroot +
                                                '_task-DynamicFaces_desc-confounds_timeseries.tsv')
    ...
    def addconfounds(self):
        ...
        self.motion_censor = self.func_dir / str(self.subj_dir.name + '_' + self.ses_dir.name + '_motion')
        subprocess.run(['1d_tool.py', '-infile', self.motionpath, '-set_nruns', '1', '-overwrite', '-show_censor_count',
                        '-censor_prev_TR', '-censor_motion', '0.25', self.motion_censor])
