...
mni_mask_2mm = Path('/fast_scratch/jdr/resting/Masks/wholeBrain/MNI_mask_2mm+tlrc.')
...
def netproc(nii):
    ...
    for amygmask in amygmasks_dir.glob('*.nii.gz'):
        ...
        wbcorr_out_unsmooth = wbcorr_dir / (subjroot + "_" + str(wbcorr_out.name).split('.')[0] + str('_unsmooth.nii.gz'))
        if wbcorr_out.exists():
            shutil.copy(wbcorr_out, wbcorr_out_unsmooth)


with parallel_backend("loky", inner_max_num_threads=1):
    results = Parallel(n_jobs=8, verbose=1)(
        delayed(netproc)(nii) for nii in sorted(rs_dir.glob('*.nii.gz')))
