...
gzip -d "${topup_dir}"/PA_AP_b0.nii.gz

topup -v --imain="${topup_dir}"/PA_AP_b0 --datain="${topup_dir}"/acqparams.txt --config=b02b0.cnf --out="${topup_dir}"/topup_PA_AP_b0
...
fslroi "${topup_dir}"/PA_b0 "${topup_dir}"/PA_b0_1st 0 1
fslroi "${topup_dir}"/AP_b0 "${topup_dir}"/AP_b0_1st 0 1

dimt=$(fslval "${topup_dir}"/PA_b0 dim4)
dimt=$(( dimt + 1 ))
...
applytopup --imain="${topup_dir}"/PA_b0_1st,"${topup_dir}"/AP_b0_1st --method=jac --topup="${topup_dir}"/topup_PA_AP_b0 --datain="${topup_dir}"/acqparams.txt --inindex=1,"${dimt}" --out="${topup_dir}"/hifib0
