...
get_options() {
  ...
  printf  "Location of study files: %s\\n" "${STUDY_DIR}"
  printf  "Performing operations for subject(s): %s\\n" "${SUBJECT}"
  printf  "Gradients tar file path is: %s\\n" "${GRADPACK}"
}
...
main() {
  ...
  #-CONVERTING the diffusion vector and diffusion weight files we get from the scanner
