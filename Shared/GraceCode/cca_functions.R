load_power_net <- function(final_clinical){
  n_sample <- dim(final_clinical)[1]
  sample_net<-array(NA, c(264, 264, n_sample))
  for (i in 1:n_sample){
    scanid <- final_clinical$scanid[i]
...
load_reg_power_net <- function(final_clinical){
  #load net
  inc_net <- load_power_net(final_clinical)
  
  # Compile covariates
  print("Compile covariates")
  final_clinical$gender.x <- as.factor(final_clinical$gender.x)
  #final_clinical$race2 <- as.factor(final_clinical$race2)
  
  # Regress
  print("Regress out covariates")
  inc_net[which(is.na(inc_net))] <- 0
  net.rgr <- apply(inc_net, 2, function(x) residuals.glm(glm(x ~ interview_age.x + 
...
ccaDW <- function(X,Y,pen_x,pen_y,rank){
  mode<-PMA::CCA(x=X, z=Y, typex=c("standard"),typez=c("standard"), penaltyx=pen_x, penaltyz=pen_y, K=rank, niter=100, trace=TRUE)
}
...
reorderCCA <- function(res,org,k){
  u <- org$u #brain matrix 
  v <- org$v #clinical matrix 
  
  res.match<-apply(abs(cor(v,res$v[,1:k])),1,which.max) #take absolute value of the correlation 

  res.cor <- apply(abs(cor(v,res$v[,1:k])),1,max)

  res.match.count <- length(unique(res.match))
  print(res.match.count)
...
brain_vis <- function(braindim,title,sign,idx, parcels){
  ccamat <- array(0,c(454,454)) #create a blank matrix
  ccamat.up <- ccamat[upper.tri(ccamat,diag=F)] #only use the top half
  ccamat.up[idx[braindim$fea]] <- braindim$load #the loadings from u.boot. plot, 
