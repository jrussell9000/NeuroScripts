...
setwd('/fast_scratch/jdr/GraceCode')
wd = '/fast_scratch/jdr/GraceCode'
##############################
#Load Packages
...
############################################
#Source cca_functions, set working directory
source('cca_functions.R')

##############################
#Load data

#Text files for clinical data and demographic data
clin_data <- fread("abcd_cbcl01_new.txt", header = TRUE)
demo <- read.table("abcddemo01.txt", header = TRUE)
...
load('med.data.RData')
load('net.data.RData')
Med_data <- read.csv("med_data.csv")
...
scca.org.alt <- scca(data$brain, data$behavior, lambda1=0.4, lambda2=0.3, standx="sd", standy="sd", ndim=modenum, verbose=TRUE, num_threads=20)
...
scca.cca <- mclapply2(seq_along(sampleid),function(i) ccaDW(brain_sample[[i]],behavior_sample[[i]],0.4,0.3,20), mc.cores=11) #loop through split
#takes a long time
...
bootnum = 100 #number of bootstraps to run (try 10)
bootid <- createResample(subjid$subjectkey, list = T, times = bootnum) #create lists of subjs for bootstrap
brain_boot <- mclapply2(bootid, function(id) {data$brain[id,]}, mc.preschedule=TRUE) #create bootstrap samples for connectivity features
...
profvis({
scca.boot <- mclapply2(seq_along(bootid),function(i) ccaDW(brain_boot[[i]],behavior_boot[[i]],0.4,0.3,10), mc.cores = 150) #run scca on these bootstrap sample
})
