...
class dwipreproc():
    ...
    def makesubjprocdirs(self):
        ...
        print("\n==Copying files to processing directory==\n")
    ...
    def fieldmapcorrection(self):
        ...
        subprocess.run(['bet2', self.mag, mag_brain, '-f', '0.3', '-v'], stdout=sys.stdout, stderr=sys.stderr)

        #Mask and correct the fieldmap
        print("\n==Masking and correcting the fieldmap (smoothing, despiking)==","\n")
        ...
        print("\n==Warping the reference (magnitude) image to the fieldmap==","\n")
        ...
        print("\n==Creating the brain-extracted mean B0 from the DWI series==","\n")
        ...
        print("\n==Registering the warped magnitude scan to the brain-extracted mean B0==","\n")
        ...
        subprocess.run(['flirt', '-v', '-in', mag_brain_warped, '-ref', meanb0_brain, '-out', mag_brain_warped_aligned, '-omat', mag_matrix, '-dof', '6'], stdout=sys.stdout, stderr=sys.stderr)
        
        #Apply the transformation matrix to the fieldmap
        print("\n==Registering the corrected fieldmap to the mean B0==","\n")
        fmap_brain_corr_warped = self.preprocdir / str(self.basename + '_fmap_brain_corr_warped.nii.gz')
        subprocess.run(['flirt', '-v', '-in', fmap_brain_corr, '-ref', meanb0_brain, '-applyxfm', '-init', mag_matrix, '-out', fmap_brain_corr_warped], stdout=sys.stdout, stderr=sys.stderr)
        
        #Undistort the DWI series
        print("==Warping the full DWI series to the fieldmap==","\n")
    ...
    def mrconvert2mif(self):
        print("\n==Converting the fieldmap corrected DWI series to .MIF==","\n")
        self.dwi_mif = self.preprocdir / str(self.basename + '_fmapcorr_dwi.mif')
        subprocess.run(['mrconvert', '-info', '-force', '-json_import', self.dwijson, '-fslgrad', self.bvec, self.bval, self.dwifmapcorr, self.dwi_mif])

    def denoise(self):
        print("\n==Denoising the fieldmap corrected DWI series==","\n")
        self.dwi_mif_den = self.preprocdir / str(self.basename + '_den.mif')
        subprocess.run(['dwidenoise', '-info', '-force', self.dwi_mif, self.dwi_mif_den])

    def degibbs(self):
        print("\n==Removing Gibbs rings from the fieldmap corrected DWI series==","\n")
        self.dwi_mif_den_deg = self.preprocdir / str(self.basename + '_den_deg.mif')
        subprocess.run(['mrdegibbs', '-info', '-force', self.dwi_mif_den, self.dwi_mif_den_deg])

    def mrconvert2nii(self):
        print("\n==Converting the fieldmap corrected DWI series back to .NII==","\n")
        self.dwi_nii_den_deg = self.procdir / str(self.basename + '_preproc_dwi.nii')
        subprocess.run(['mrconvert', '-info', '-force', self.dwi_mif_den_deg, self.dwi_nii_den_deg])
